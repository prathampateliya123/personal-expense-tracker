...
import { INITIAL_INCOME_FILTERS } from "../../services/incomeService";
...
const IncomeAvatar = ({ category, colorMap }) => (
  <div
...
const ActionButtons = ({ income, onDelete }) => (
...
const IncomeRow = ({ income, onDelete, colorMap }) => (
  <tr className="group border-b border-border transition last:border-0 hover:bg-surfaceLight/70">
    <td className="px-5 py-4">
      <div className="flex min-w-[220px] items-center gap-3">
        <IncomeAvatar category={income.category} colorMap={colorMap} />
        <div className="min-w-0">
          <p className="truncate font-semibold text-textPrimary" title={income.title}>
            {income.title}
          </p>
          {income.description ? (
            <p className="mt-0.5 truncate text-xs text-textSecondary" title={income.description}>
              {income.description}
            </p>
          ) : (
            <p className="mt-0.5 text-xs text-textSecondary">{income.paymentMode}</p>
          )}
        </div>
      </div>
    </td>
    <td className="px-5 py-4">
      <span
        className={`category-chip whitespace-nowrap ${getCategoryChipClass(
          colorMap?.[income.category] || income.category
        )}`}
      >
        {income.category}
      </span>
    ...
    <td className="px-5 py-4">
      <PaymentBadge mode={income.paymentMode} />
    </td>
    ...
    <td className="px-5 py-4">
      <ActionButtons income={income} onDelete={onDelete} />
    </td>
  </tr>
);

const IncomeMobileCard = ({ income, onDelete, colorMap }) => (
    ...
    <div className="flex items-start gap-3">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <p className="truncate font-semibold text-textPrimary">{income.title}</p>
            <p className="mt-0.5 text-xs text-textSecondary">
              {formatExpenseDate(income.date)} · {income.paymentMode}
            </p>
          </div>
...
const IncomeTable = ({
  ...
  const handleClear = () => {
    setSearchInput("");
    setDebouncedSearch("");
    onFiltersChange({ ...INITIAL_INCOME_FILTERS });
  };
  ...
  const handleDeleteClick = (income) => setDeleteTarget(income);
  ...
  const showTableContent = !loading && incomes.length > 0;
  const showEmpty = !loading && incomes.length === 0;
