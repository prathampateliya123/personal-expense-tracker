...
import { CalendarDaysIcon, ChevronDownIcon, ChevronLeftIcon, ChevronRightIcon, XMarkIcon } from "./Icons";
import { MessageBox } from "./MessageBox";
import { DATE_OPERATOR_OPTIONS, addMonths, clampToMaxDay, clampToMaxMonth, detectPresetId, endOfMonth, formatDateFilterLabel, formatDisplayDate, formatIsoDate, getMonthMatrix, getMonthOptions, getPresetsForOperator, isAfterDay, isBeforeDay, isSameDay, isWithinInclusive, resolvePresetRange, startOfMonth, toDateOnly } from "../../utils/report";
import { UNDERSCORE_REGEX } from "../../utils/constants";
...
function HeaderSelectButton({
  ...
  return (
    <button
      ...
      aria-expanded={open}
      onClick={onClick}
      className={`inline-flex min-w-0 items-center justify-between gap-1.5 rounded-[7px] border bg-[var(--surface)] text-left font-semibold text-[var(--ink)] outline-none transition-[border-color,box-shadow] ${
        open
  ...
  return (
    <div ref={rootRef} className={`relative min-w-0 ${className}`}>
      ...
      {open ? (
        <div
          role="listbox"
          className="absolute left-0 right-0 top-[calc(100%+4px)] z-50 max-h-52 overflow-y-auto overscroll-contain rounded-[7px] border border-[var(--border)] bg-[var(--surface)] py-1 shadow-[0_12px_28px_rgba(17,24,39,0.16)]"
        >
          {options.map((option) => {
  ...
  return (
    <button
      ...
      className={`relative flex h-full min-h-0 w-full touch-manipulation items-center justify-center text-[12px] sm:text-[13px] transition-colors select-none ${
        disabled
          ? "cursor-not-allowed text-[var(--ink-subtle)]/45"
          : inCurrentMonth
            ? "cursor-pointer text-[var(--ink)]"
            : "cursor-pointer text-[var(--ink-subtle)]"
      }`}
    >
      {showDimSelected ? (
        <span className="pointer-events-none absolute inset-y-1 left-0 right-0 bg-[var(--brand-orange)]/15" />
      ) : null}

      {showPreview ? (
        <span
          className={`pointer-events-none absolute inset-y-1 left-0 right-0 ${isPreviewStart && isPreviewEnd
              ? "rounded-full bg-[var(--brand-orange)]"
              : isPreviewStart
                ? "rounded-l-full bg-[var(--brand-orange)]"
                : isPreviewEnd
                  ? "rounded-r-full bg-[var(--brand-orange)]"
                  : "bg-[var(--brand-orange)]/70"
            }`}
        />
      ) : null}
      ...
      <span
        className={`pointer-events-none relative z-[1] inline-flex h-7 w-7 sm:h-8 sm:w-8 items-center justify-center rounded-full ${inFill
            ...
            : ""
          } ${isToday && !inFill ? "underline decoration-[var(--brand-orange)] decoration-2 underline-offset-4" : ""}`}
      >
        {day.getDate()}
      </span>
    </button>
...
function MonthCalendar({
  ...
  const pickerCellClass = (active, disabled = false) =>
    `flex h-full min-h-0 w-full items-center justify-center rounded-[7px] text-[13px] font-semibold transition-colors ${
      disabled
        ? "cursor-not-allowed text-[var(--ink-subtle)]/40"
        : active
          ? "cursor-pointer bg-[var(--brand-orange)] text-white"
          : "cursor-pointer text-[var(--ink)] hover:bg-[var(--brand-orange-soft)] hover:text-[var(--brand-orange-strong)]"
    }`;
        ...
        {openPicker === "month" ? (
          ...
          <div className="flex h-full min-h-0 flex-col">
            <div className="mb-1 flex shrink-0 items-center justify-between gap-2 px-0.5">
              ...
              <p className="text-[12px] font-semibold text-[var(--ink-muted)]">
              ...
              <button
                ...
                className={`inline-flex h-7 w-7 items-center justify-center rounded-[6px] transition-colors ${
                  canGoNextYearPage
                    ? "cursor-pointer text-[var(--ink-muted)] hover:bg-[var(--brand-orange-soft)] hover:text-[var(--brand-orange-strong)]"
                    : "cursor-not-allowed text-[var(--ink-subtle)]/40"
                }`}
          ...
          <div className="flex h-full min-h-0 flex-col">
            <div className="mb-1 grid shrink-0 grid-cols-7 gap-0">
              {WEEKDAYS.map((day) => (
                <div
                  key={day}
                  className="py-1 text-center text-[10px] sm:text-[11px] font-semibold uppercase tracking-[0.04em] text-[var(--ink-subtle)]"
                >
                  {day.slice(0, 2)}
                </div>
              ))}
  ...
  if (Array.isArray(operators) && operators.length) {
    const list = operators
      .map((item) => {
        ...
        return {
          value,
          label: String(item?.label || value).replace(UNDERSCORE_REGEX, " ")
        };
      })
      .filter(Boolean);
...
export default function DateRangePicker({
  ...
  const handleApply = () => {
    if (draftOperator === "between") {
      ...
      if (showApplyToast) MessageBox("success", "Date filter applied");
      setOpen(false);
      return;
    }
        ...
        className={
          compact
            ? `shrink-0 rounded-full px-3 py-1.5 text-[12px] font-semibold transition-colors cursor-pointer ${active
              ? "bg-[var(--brand-orange-soft)] text-[var(--brand-orange)] ring-1 ring-[var(--brand-orange)]/35"
              : "bg-[var(--canvas)] text-[var(--ink)]"
            }`
            : `flex w-full min-w-0 cursor-pointer items-center px-3.5 py-2.5 text-left text-[13px] transition-colors border-b border-[var(--canvas)] last:border-b-0 ${active
              ? "bg-[var(--brand-orange-soft)] font-semibold text-[var(--brand-orange)]"
              : hovered
                ? "bg-[var(--canvas)] font-semibold text-[var(--brand-orange-strong)]"
  ...
  return (
    <div ref={rootRef} className={`relative min-w-0 ${className}`}>
      <button
        ...
        className={`inline-flex h-[42px] w-full max-w-full items-center gap-2 rounded-[7px] border bg-[var(--surface)] px-2.5 sm:px-3 text-left text-[12px] sm:text-[13px] font-semibold transition-colors hover:border-[var(--ink-subtle)] cursor-pointer ${hasActiveDateFilter
            ? "border-[var(--brand-orange)] text-[var(--ink)] bg-[var(--brand-orange-soft)]/30"
        ...
        title={appliedLabel}
      >
        <CalendarDaysIcon className="h-[18px] w-[18px] shrink-0 text-[var(--brand-orange)]" aria-hidden />
        ...
        {hasActiveDateFilter ? (
          <span
            ...
            className="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-[var(--ink-muted)] hover:bg-[var(--brand-orange)]/20 hover:text-[var(--brand-orange-strong)] transition-colors cursor-pointer"
      ...
      <AnimatePresence>
        {open && isMobileSheet ? (
          <motion.div
            ...
            className="fixed inset-0 z-[70] bg-[var(--sidebar-bg)]/45 sm:hidden"
        ...
        {open ? (
          <motion.div
            ...
            className={`z-[9999] box-border flex max-h-[inherit] min-h-0 min-w-0 flex-col overflow-hidden border border-[var(--border)] bg-[var(--surface)] shadow-[0_18px_50px_rgba(17,24,39,0.16)] ${isMobileSheet ? "rounded-t-[16px] border-b-0" : "rounded-[12px]"
              }`}
          ...
          >
            ...
            >
              <aside
                className={`flex w-full shrink-0 flex-col border-b border-[var(--border)] ${draftOperator === "between"
                    ? "md:min-h-0 md:w-[180px] md:border-b-0 md:border-r md:border-[var(--border)] lg:w-[200px]"
                    : "sm:min-h-0 sm:w-[150px] sm:border-b-0 sm:border-r sm:border-[var(--border)] md:w-[160px]"
                  }`}
              >
                {!hideOperatorSelect ? (
                  <div className="shrink-0 border-b border-[var(--border)] p-2.5 sm:p-3">
                    <label className="block">
                      <span className="mb-1 block text-[11px] font-semibold uppercase tracking-[0.06em] text-[var(--ink-subtle)]">
                        Filter type
                      </span>
                      <AsideFieldSelect
                        value={draftOperator}
              ...
              <div className="flex min-h-0 min-w-0 flex-1 flex-col">
                    ...
                    <div className="mb-3 grid grid-cols-2 gap-2 sm:mb-4">
                      <div
                        className={`min-w-0 rounded-[7px] border px-2.5 py-2 sm:px-3 sm:py-2.5 ${pickingEnd || !draftStart
                            ...
                            : "border-[var(--brand-orange)] shadow-[0_0_0_1px_rgba(246,143,61,0.15)]"
                          }`}
                      >
                        <p className="text-[10px] sm:text-[11px] font-semibold uppercase tracking-[0.06em] text-[var(--ink-subtle)]">
                          Start
                        </p>
                        <p className="mt-0.5 truncate text-[13px] sm:text-[15px] font-semibold text-[var(--ink)]">
                  ...
                  {draftOperator === "between" ? (
                    <div className="mb-2 flex items-center justify-between">
                      <button
                        ...
                        className="inline-flex h-8 w-8 items-center justify-center rounded-[7px] border border-[var(--border)] text-[var(--ink-muted)] transition-colors hover:bg-[var(--canvas)] cursor-pointer"
                      ...
                      <button
                        ...
                        className={`inline-flex h-8 w-8 items-center justify-center rounded-[7px] border border-[var(--border)] transition-colors ${
                          canGoNextMonth
                            ? "cursor-pointer text-[var(--ink-muted)] hover:bg-[var(--canvas)]"
                            : "cursor-not-allowed text-[var(--ink-subtle)]/40"
                        }`}
                ...
                <div className="flex shrink-0 flex-col-reverse gap-2 border-t border-[var(--canvas)] bg-[var(--surface)] px-3 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-4">
                  <button
                    type="button"
                    onClick={handleClear}
                    disabled={!canClear}
                    className="inline-flex h-[38px] w-full items-center justify-center rounded-[7px] border border-[var(--border-strong)] bg-[var(--surface)] px-3 text-[13px] font-semibold text-[var(--ink-muted)] transition-colors hover:border-[var(--ink-subtle)] hover:bg-[var(--canvas)] hover:text-[var(--ink)] cursor-pointer disabled:cursor-not-allowed disabled:opacity-40 sm:w-auto sm:px-4"
                  >
                    Clear Filter
                  </button>
                  <div className="flex w-full items-center gap-2 sm:w-auto">
                    <button
                      ...
                      className="inline-flex h-[38px] flex-1 items-center justify-center rounded-[7px] border border-[var(--border-strong)] bg-[var(--surface)] px-4 text-[13px] font-semibold text-[var(--ink)] transition-colors hover:bg-[var(--canvas)] cursor-pointer sm:flex-none"
                    >
                      Cancel
                    </button>
                    <button
                      ...
                      onClick={handleApply}
                      disabled={!canApply}
                      className="inline-flex h-[38px] flex-1 items-center justify-center rounded-[7px] bg-[var(--brand-orange)] px-4 text-[13px] font-semibold text-white transition-opacity hover:opacity-90 cursor-pointer disabled:cursor-not-allowed disabled:opacity-50 sm:flex-none"
                    >
                      Apply
                    </button>
                  </div>
